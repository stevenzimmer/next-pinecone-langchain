...
export default function Home() {
  ...
  async function sendQuery() {
    ...
    try {
      ...
      console.log({error});
      setResult("Something went wrong with your query, please try again");
      setLoading(false);
    }
  }
    ...
    <main className="container mx-auto h-screen">
      <div className="flex flex-wrap h-full">
        <div className="w-full lg:w-6/12">
          ...
          <textarea onChange={(e) => setQuery(e.target.value)} className="text-black p-3 rounded-md w-full mb-6 border-2" /><br />
        ...
        <div className="w-full lg:w-6/12">
          <div className="bg-white h-full p-12 drop-shadow-xl overflow-y-scroll">
            ...
            <h3 className="text-2xl text-center font-bold">Recent Questions </h3>
           
            {answers.length > 0 && (
              <>
          {answers.map((answer, i) => {
            return (
              <div key={`query-${i}`} className="py-6 border-b">
                <p className="mb-2"><strong> {answer.question}</strong></p>
